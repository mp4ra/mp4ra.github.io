...
def getCSV4CCs(directory):
    ...
    for fileName in os.listdir(directory):
        ...
        if (
            ...
            and fileName != "oti.csv"
            and fileName != "stream-types.csv"
            and fileName != "unlisted.csv"
            and fileName != "textualcontent.csv"
            and fileName != "knownduplicates.csv"
        ):
...
def notfourcharacters(codes, exceptions=[]):
    ...
    if mistakeCodes == []:
        ...
        for i in mistakeCodes:
            print("ERROR: '%s' from '%s' is not a valid fourcc" % (i[0], i[1]))
...
def registerspecs(codesInCSV, speclist, specexceptions=[]):
    ...
    if unregisteredspecs == []:
        ...
        for i in unregisteredspecs:
            print(
                "ERROR: '%s' from '%s' with '%s' specification is either non-existent or invalid"
                % (i[0], i[3], i[2])
            )
...
def filledcolumns(codesInCSV):
    ...
    if newmissingcols == set():
        ...
        for b in newmissingcols:
            print("ERROR: '%s' from '%s' has missing columns" % (str(b[0]), str(b[3])))
...
def registerhandle(codesInCSV, handlerexceptions):
    unregisteredhandles = []
    allhandles = [
        handle[1] for handle in codesInCSV if handle[3] == "handlers.csv"
    ] + handlerexceptions
    for a in range(len(codesInCSV)):
...
def prsanitycheck():
    ...
    # Exit Codes
    returnvalue = (
        not4CCs + duplicates + unregisteredspecs + emptycols + unregisteredhandles
    )
    if returnvalue == 0:
